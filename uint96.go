...
func u96FromBig(x *big.Int) (Uint96, bool) {
	switch {
	case x.Sign() == 0:
		return Uint96{}, true
	case x.IsUint64():
		return U96(x.Uint64()), true
	case x.BitLen() > 96:
		return Uint96{}, false
	}
	var u0, u1 uint64
	if w := x.Bits(); bits.UintSize == 32 {
		u0 = uint64(w[0]) | uint64(w[1])<<32
		u1 = uint64(w[2])
	} else {
		u0 = uint64(w[0])
		u1 = uint64(w[1])
	}
	if u1 > math.MaxUint32 {
		return Uint96{}, false
	}
...
func (x Uint96) uint256() Uint256 {
	return Uint256{x.u0, uint64(x.u1), 0, 0}
}
...
func (x Uint96) mulPow10(n uint) (Uint96, bool) {
	switch {
	case x.IsZero():
		return Uint96{}, true
	case n == 0:
		return x, true
	case n >= uint(len(pow10tab96)):
		return Uint96{}, false
	default:
		return x.MulCheck(pow10tab96[n])
	}
}
